...
public class CustomUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        return userService.findOneWithAuthoritiesByUsername(username)
    ...
    private org.springframework.security.core.userdetails.User createUser(String username, UserModel userModel){
        if(!userModel.isUser_activated()){
            logger.debug("아이디가 활성화 되어 있지 않습니다.");
            throw new RuntimeException(username + "=-> 활성화되어 있지 않습니다.");
        }
        assert userModel.getAuthorities() != null;
        List<GrantedAuthority> grantedAuthorities = userModel.getAuthorities().stream()
                .map(authority -> new SimpleGrantedAuthority(authority.getAuthority_name())) // 가져온 user 의 권한 이름으로 권한객체 생성
                .collect(Collectors.toList());
        logger.debug("UserDetails 로 만들어줌");
        return new org.springframework.security.core.userdetails.User(userModel.getUser_id(),userModel.getUser_pw(),grantedAuthorities);
